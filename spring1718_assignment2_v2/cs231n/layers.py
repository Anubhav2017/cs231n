...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        running_mean = momentum * running_mean + (1 - momentum) * sample_mean
        running_var = momentum * running_var + (1 - momentum) * sample_var
        cache=(sample_mean,sample_var,bn_param,gamma)
...
def batchnorm_backward(dout, cache):
    ...
    dva2=gamma*dout
    dgamma=np.sum(dout*dva2,axis=0)
    dx=dva2/np.sqrt(svar+eps)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for m in range(0,N):
    	for f in range(0,F):
    		for i in range(0,Hdash):
    			for j in range(0,Wdash):
    				ibegin=i*stride
    				jbegin=j*stride
    				window=xpad[m,:,np.arange(ibegin,ibegin+HH),np.arange(jbegin,jbegin+WW)]
    				out[m,f,i,j]=np.sum(window*w[f]+b[f],axis=None)
    	...
    	for j in range(0,Wdash):
    		for c in range(0,C):
    			for m in range(0,N):
    				istart=i*s
    				jstart=j*s
    				out[m,c,i,j]=np.max(x[m,c,istart:istart+pool_height,jstart:jstart+pool_width])
    	...
    	for j in range(0,Wdash):
    		for c in range(0,C):
    			for m in range(0,N):
    				...
    				imax=ibegin
    				jmax=jbegin
    				maxel=x[m,c,imax,jmax]
    				for a in range(ibegin,ibegin+pool_height):
    					for b in range(jbegin,jbegin+pool_width):
    						if(x[m,c,a,b]>maxel):
    							imax=a
    							jmax=b
    							maxel=x[m,c,a,b]
    				dx[m,c,imax,jmax]=maxel*dout[m,c,i,j]
